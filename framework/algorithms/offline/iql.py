...
class Algorithm:
    ...
    def __init__(self, args, device, dtype, stats_path):
        ...
        self.alpha = float(getattr(args, "iql_alpha", 0.005))                # target network update rate (Polyak averaging coefficient)
        ...
        # target critic (for stability with slow-moving target)
        ...
        print(f"With both twin Q and polyak target update")
    ...
    def train_step(self, data, it):
        ...
        # twin critics on (s,a)_0 for Q value estimation
        q1_logits = agent(s_a_0, which="q1")  # (n0,)
        q2_logits = agent(s_a_0, which="q2")  # (n0,)
        # qmin_logits = torch.min(q1_logits, q2_logits)

        # target critic on twin critics on (s,a)_0 for stable Q value estimation in V and policy loss
        with torch.no_grad():
            qt1_logits = target_agent(s_a_0, which="q1")  # (n0,)
            qt2_logits = target_agent(s_a_0, which="q2")  # (n0,)
            qtmin_logits = torch.min(qt1_logits, qt2_logits)
        ...
        for sz0, sz1 in zip(s0_sizes, s1_sizes):
            ...
            # V(s') as the bootstrapped target value (state repeats across actions -> mean is ok and even could be each of them)
            t_list.append(v1_logits[start1:end1].mean())

            # ---------- V loss: expectile regression ----------
            # LV = E[ L^tau_2( Q_target_min(s,a) - V(s) ) ]
            u = qtmin_logits[start0:end0].detach() - v0_logits[start0:end0]
            vloss_list.append(self._expectile_loss(u, self.tau).mean())

            # ---------- Policy loss: AWR extraction ----------
            # Lpi = E[ exp(beta*(Q_target_min - V)) * log pi(a|s) ]
            ...
            adv = (qtmin_logits[start0:end0].detach() - v0_logits[start0:end0].detach())
        ...
        for gs in group_sizes:
            ...
            """
            # q_losses.append(0.5 * (
            #     (q1_slice - t_slice).pow(2).mean() +
            #     (q2_slice - t_slice).pow(2).mean()
            # ))            
            q_losses.append(0.5 * (
                (q1_slice - t_slice).mean().pow(2) +
                (q2_slice - t_slice).mean().pow(2)
            ))
        ...
        # ----------------  target update  ----------------------
        soft_update(agent, target_agent, self.alpha)
