...
class Algorithm:
    ...
    def train_step(self, data, it):
        ...
        # =========================
        # 0) Model forward passes to get all necessary values for the losses
        # =========================
        ...
        with torch.no_grad():
            ...
            v1_logits = target_agent(s1, which="v")      # (n1,)
        ...
        q1_list, q2_list, qmin_list, t_list, v0_list, v1_list = [], [], [], [], [], []
        start0 = 0
        start1 = 0
        ...
        v1 = torch.stack(v1_list).unsqueeze(1).to(device)
        
        # reward contribution from q
        td1_r_g = q1_value - r_event - gammas * t_value 
        td2_r_g = q2_value - r_event - gammas * t_value 

        # reward contribution from v
        tdv_r_g = v0 - r_event - gammas * v1.detach()
        ...
        for gs in group_sizes:
            ...
            # for v loss, we do the same group-based mean aggregation approach for consistency with the TD loss, even though it's not strictly necessary for the V loss calculation since V is only state-based. Alternatively, it could be calculated using the segment TD error with r)_group contribution.
            """
            ...
            # V loss per group
            # LV = E[ L_2( v(s) - r_group - gamma*V(s') ) ] with the same group-based mean aggregation approach as above for consistency with the TD loss
            tdv_g = tdv_r_g[sl].mean() - shared_r_group
            v_loss_g = tdv_g.pow(2)
        ...
        # =========================
        # 4) PPO actor update using twin-Q advantage and v baseline
        # =========================
        ...
        # =========================
        # 4)  stats calculation and recording
        # =========================
        ...
        return {
            "loss": float((critic_loss + actor_loss + v_loss).detach().cpu().item()),
